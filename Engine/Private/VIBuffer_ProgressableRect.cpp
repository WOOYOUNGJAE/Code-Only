...
CVIBuffer_ProgressableRect::CVIBuffer_ProgressableRect(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CVIBuffer_Rect(pGraphic_Device), m_hBaseTexHandle{}, m_hBaseTexDesc{}, m_hRatio{}
{
}

CVIBuffer_ProgressableRect::CVIBuffer_ProgressableRect(const CVIBuffer_ProgressableRect& rhs)
	: CVIBuffer_Rect(rhs), m_hBaseTexHandle{ rhs.m_hBaseTexHandle }, m_hBaseTexDesc{ rhs.m_hBaseTexDesc }
	, m_hRatio(rhs.m_hRatio)
{
}

HRESULT CVIBuffer_ProgressableRect::Initialize_Prototype()
{
...
HRESULT CVIBuffer_ProgressableRect::Initialize(void* pArg)
{
...
HRESULT CVIBuffer_ProgressableRect::Render()
{
...
HRESULT CVIBuffer_ProgressableRect::Create_VertexShader()
{
...
HRESULT CVIBuffer_ProgressableRect::Create_PixelShader()
{
	ID3DXBuffer* pPixelShader = nullptr;
	ID3DXBuffer* errorBuffer = nullptr;

	HRESULT hr = D3DXCompileShaderFromFile(
		L"../Shader/ProgressPixelShader.hlsl",
		0,
		0,
		"main",
		"ps_3_0",
...
void CVIBuffer_ProgressableRect::Initialize_ConstantTable()
{
...
CVIBuffer_ProgressableRect* CVIBuffer_ProgressableRect::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CVIBuffer_ProgressableRect* pInstance = new CVIBuffer_ProgressableRect(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CVIBuffer_ProgressableRect");
	}

	return pInstance;
}

CComponent* CVIBuffer_ProgressableRect::Clone(void* pArg)
{
	CVIBuffer_ProgressableRect* pInstance = new CVIBuffer_ProgressableRect(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CVIBuffer_ProgressableRect");
	}

	return pInstance;
}

void CVIBuffer_ProgressableRect::Free()
{
	__super::Free();
