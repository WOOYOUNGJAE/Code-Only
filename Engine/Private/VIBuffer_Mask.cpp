...
CVIBuffer_Mask::CVIBuffer_Mask(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CVIBuffer_Rect(pGraphic_Device), m_hBaseTexHandle{}, m_hBaseTexDesc{}, m_hRadius{}
{
}

CVIBuffer_Mask::CVIBuffer_Mask(const CVIBuffer_Mask& rhs)
	: CVIBuffer_Rect(rhs), m_hBaseTexHandle{ rhs.m_hBaseTexHandle }, m_hBaseTexDesc{ rhs.m_hBaseTexDesc }
	, m_hRadius(rhs.m_hRadius)
{
}

HRESULT CVIBuffer_Mask::Initialize_Prototype()
{
...
HRESULT CVIBuffer_Mask::Initialize(void* pArg)
{
...
HRESULT CVIBuffer_Mask::Create_VertexShader()
{
...
HRESULT CVIBuffer_Mask::Create_PixelShader()
{
	ID3DXBuffer* pPixelShader = nullptr;
	ID3DXBuffer* errorBuffer = nullptr;

	HRESULT hr = D3DXCompileShaderFromFile(
		L"../Shader/MaskPixelShader.hlsl",
		0,
		0,
		"main",
		"ps_3_0",
...
void CVIBuffer_Mask::Initialize_ConstantTable()
{
	...
	m_hRadius = m_pPixelConstantTable->GetConstantByName(0, "g_Radius");

	m_hViewportSizeX = m_pPixelConstantTable->GetConstantByName(0, "g_ViewportSizeX");
	m_hViewportSizeY = m_pPixelConstantTable->GetConstantByName(0, "g_ViewportSizeY");
...
CVIBuffer_Mask* CVIBuffer_Mask::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CVIBuffer_Mask* pInstance = new CVIBuffer_Mask(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Created : CVIBuffer_Mask");
	}

	return pInstance;
}

CComponent* CVIBuffer_Mask::Clone(void* pArg)
{
	CVIBuffer_Mask* pInstance = new CVIBuffer_Mask(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		Safe_Release(pInstance);
		MSG_BOX("Failed to Cloned : CVIBuffer_Mask");
	}

	return pInstance;
}

void CVIBuffer_Mask::Free()
{
	__super::Free();
